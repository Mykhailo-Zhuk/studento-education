  ...
  return (
    <>
      <div className="md:hidden space-y-3">
        {loading && (
          <p className="text-center text-[14px] text-text-muted py-8">
            Loading…
          </p>
        )}
        ...
        {rows.map((g) => (
          <article
            key={g.id}
            className="bg-white border border-border-light rounded-xl shadow-sm overflow-hidden"
          >
            <div className="flex items-center gap-3 px-4 py-3 border-b border-border-light">
              <div
                className={`w-9 h-9 rounded-lg ${g.iconBg} flex items-center justify-center ${g.iconColor} shrink-0`}
              >
                <g.icon size={18} />
              </div>
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-2 mt-0.5">
                  <span
                    className={`text-[11px] px-2 py-0.5 rounded-full font-semibold ${g.typeClasses}`}
                  >
                    {g.type}
                  </span>
                  <span
                    className={`text-[11px] px-2 py-0.5 rounded-full font-semibold ${g.isActive ? "bg-success-light text-success" : "bg-surface-gray-light text-text-muted"}`}
                  >
                    {g.status}
                  </span>
                </div>
              ...
              <button
                ...
                className="p-2 rounded-lg text-text-muted hover:text-primary hover:bg-surface-gray-light transition-colors shrink-0"
              >
            ...
            <div className="px-4 py-3 grid grid-cols-2 gap-x-4 gap-y-2 text-[13px]">
              <div>
                <span className="text-text-muted text-[11px]">Members</span>
                <p className="text-text-primary truncate">{g.members}</p>
              </div>
              <div>
                <span className="text-text-muted text-[11px]">Schedule</span>
                <p className="text-text-primary truncate">{g.schedule}</p>
              </div>
              <div>
                <span className="text-text-muted text-[11px]">Started</span>
                <p className="text-text-primary">{g.started}</p>
              </div>
              <div>
                <span className="text-text-muted text-[11px]">Finished</span>
                <p className="text-text-primary">{g.finished}</p>
              </div>
            </div>
            {(g.journalUrl ||
              g.telegramUrl ||
                ...
                >
                    ...
                    <span
                      className={`text-[13px] px-2 py-1 rounded-full font-semibold ${
                        g.isActive
                          ...
                          : "bg-surface-gray-light text-text-muted"
                      }`}
                    >
                      {g.status}
                    </span>
                  ...
                  <td className="px-4 whitespace-nowrap">
                    <span
                      ...
                      onMouseEnter={(e) =>
                        onShowTooltip(e, "Members", g.members)
                      }
