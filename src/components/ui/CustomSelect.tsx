...
export function CustomSelect({
  ...
  return (
    <div ref={ref} className="relative">
      ...
      {open && (
        <ul className="absolute left-0 right-0 top-full mt-1 z-10 bg-white border border-border-light rounded-lg shadow-lg max-h-52 overflow-y-auto">
          {options.map((o, i) => (
            <li key={o.value}>
              {i > 0 && <div className="mx-3 border-t border-border-light opacity-50" />}
              <button
                type="button"
